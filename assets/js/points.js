...
export function validatePoints(firstCard, secondCard) {
  // Verifica se as cartas existem
  if (!firstCard || !secondCard) {
    console.error("Cartas não encontradas");
    return;
  }

  // Compara os data-card-value das cartas para verificar se são iguais
  const idImageSelected1 = firstCard.dataset.cardValue;
  const idImageSelected2 = secondCard.dataset.cardValue;
